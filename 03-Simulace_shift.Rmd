...
```{r fig.cap="Points that define the interpolation polynomials."}
# Graph of the points
...
```{r fig.cap='Representation of two functions over the interval $[0, 1]$, from which we generate observations from classes 0 and 1.', include=FALSE}
...
```{r fig.cap="The progression of $GCV(\\lambda)$ for the selected vector $\\boldsymbol\\lambda$. The values on the x-axis are plotted on a logarithmic scale. The optimal value of the smoothing parameter $\\lambda_{optimal}$ is shown in red."}
...
```{r fig.cap='Plot of all smoothed observed curves, colored according to their classification class. The thick line represents the mean for each class.', warning=FALSE}
DFsmooth <- data.frame(
  t = rep(t, 2 * n),
...
```{r fig.cap='Plot of all smoothed observed curves, colored according to their classification class. The thick line represents the mean for each class. Zoomed-in view.', warning=FALSE}
DFsmooth <- data.frame(
  t = rep(t, 2 * n),
  time = rep(rep(1:n, each = length(t)), 2),
  Smooth = c(fdobjSmootheval),
...
We can see that the optimal parameter $K$ is `r K.opt` with an error rate calculated using 10-fold CV of `r round(1 - accuracy.opt.cv, 4)`. For clarity, let's also plot the validation error as a function of the number of neighbors $K$.
...
Thus, we see that the error rate of the model constructed using the $K$ nearest neighbors method with the optimal choice $K_{optimal}$ equal to `r K.opt`, which we determined through cross-validation, is `r 1 - round(neighb.model$max.prob, 4)` on the training data and `r round(1 - accuracy, 4)` on the test data.
...
In this specific case, we have taken the number of principal components as $p$ = `r nharm`, which together explain `r 100 * round(cumsum(data.PCA$varprop)[nharm], 4)` % of the variability in the data. The first principal component explains `r 100 * round(data.PCA$varprop[1], 4)` % and the second `r 100 * round(data.PCA$varprop[2], 4)` % of the variability. We can graphically display the scores of the first two principal components, color-coded by classification class membership.

```{r fig.cap="Scores of the first two principal components for the training data. Points are color-coded based on class membership."}
...
To determine the classification accuracy on the test data, we need to compute the scores for the first `r nharm` principal components for the test data. These scores can be calculated using the formula:
...
We have calculated the error rate of the classifier on the training data (`r 100 * round(1-presnost.train, 4)` %) as well as on the test data (`r 100 * round(1-presnost.test, 4)` %).
...
```{r fig.cap="Scores of the first two principal components, color-coded according to class membership. The decision boundary (line in the plane of the first two principal components) between the classes constructed using LDA is shown in black."}
...
# Principal component analysis ... identical to LDA
...
Thus, we have calculated the error rate of the classifier on the training data (`r 100 * round(1-presnost.train, 4)` %) as well as on the test data (`r 100 * round(1-presnost.test, 4)` %).
...
```{r fig.cap="Scores of the first two principal components, color-coded according to class membership. The decision boundary (parabola in the plane of the first two principal components) between the classes constructed using QDA is shown in black."}
...
Let’s also plot the validation error rate along with the highlighted optimal value $n_{optimal}$ as `r n.basis.opt` with validation error rate `r round(1 - presnost.opt.cv, 4)`.

```{r fig.cap='Dependence of validation error rate on the value of $n_{basis}$, that is, on the number of bases.'}
...
We have calculated the training error rate (which is `r 100 * (1 - round(presnost.train, 4))` %) and the test error rate (which is `r 100 * (1 - round(presnost.test, 4))` %). To better visualize the estimated probabilities of belonging to the classification class $Y = 1$ on the training data in relation to the values of the linear predictor, we can plot:
...
```{r fig.cap="Scores of the first two principal components, color-coded according to classification class membership. The decision boundary (line or curves in the plane of the first two principal components) between the classes is marked in black, constructed using the SVM method."}
...
The error rate of the SVM method applied to the base coefficients on the training data is therefore `r 100 * round(1-presnost.train.l, 4)` % for the linear kernel, `r 100 * round(1-presnost.train.p, 4)` % for the polynomial kernel, and `r 100 * round(1-presnost.train.r, 4)` % for the radial kernel. On the test data, the error rate of the method is `r 100 * round(1-presnost.test.l, 4)` % for the linear kernel, `r 100 * round(1-presnost.test.p, 4)` % for the polynomial kernel, and `r 100 * round(1-presnost.test.r, 4)` % for the radial kernel.
...
We can see that the best value of the parameter $d$ is `r d.opt[1]` for the linear kernel with the error value calculated using 10-fold CV `r round(1-presnost.opt.cv[1], 4)`, `r d.opt[2]` for the polynomial kernel with the value calculated using 10-fold CV `r round(1-presnost.opt.cv[2], 4)`, and `r d.opt[3]` for the radial kernel with the value `r round(1-presnost.opt.cv[3], 4)`.  
For clarity, let's also plot the progression of validation errors depending on the dimension $d$.

```{r fig.cap='Dependency of validation error on the dimension of the subspace $V_d$, separately for all three considered kernels in the SVM method. The optimal values of the dimension $V_d$ for each kernel function are marked by black dots.'}
...
The error rate of the SVM method applied to the basis coefficients on the training data is thus `r 100 * round(Res[1, 2], 4)` % for the linear kernel, `r 100 * round(Res[2, 2], 4)` % for the polynomial kernel, and `r 100 * round(Res[3, 2], 4)` % for the Gaussian kernel.  
...
Now we can compute the representations of individual curves. First, let's choose $\hat d$ as the full dimension, i.e., $\hat d = m ={}$ `r dim(alpha.RKHS)[1]`, and then determine the optimal $\hat d$ using cross-validation.
...
```{r echo=FALSE}
knitr::kable(Res,
             caption = "Summary results of the SVM method combined with RKHS on simulated data. $\\widehat{Err}_{train}$ indicates the estimated training error and $\\widehat{Err}_{test}$ indicates the test error.",
             digits = 4,
             col.names = c('Model                            ', 
...
knitr::kable(df.RKHS.res,
             caption = "Summary results of cross-validation for the SVM method combined with RKHS on simulated data. $\\widehat{Err}_{train}$ denotes the estimate of training error and $\\widehat{Err}_{test}$ denotes the testing error.",
             digits = 4,
...
We see that the optimal parameter values are $d={}$ `r d.opt[1]` and $\gamma={}$ `r round(gamma.opt[1], 4)` for the linear kernel with an error value computed using 10-fold CV `r round(err.opt.cv[1], 4)`, $d={}$ `r d.opt[2]` and $\gamma={}$ `r round(gamma.opt[2], 4)` for the polynomial kernel with an error value computed using 10-fold CV `r round(err.opt.cv[2], 4)`, and $d={}$ `r d.opt[3]` and $\gamma={}$ `r round(gamma.opt[3], 4)` for the radial kernel with an error value of `r round(err.opt.cv[3], 4)`. For interest, let’s plot the validation error function as a function of dimension $d$ and hyperparameter $\gamma$.
...
```{r echo=FALSE}
knitr::kable(Res,
             caption = "Summary results of the SVM method combined with RKHS on simulated data. $\\widehat{Err}_{train}$ denotes the estimate of training error and $\\widehat{Err}_{test}$ denotes the testing error.",
             digits = 4,
             col.names = c('Model                            ', 
                           '$\\widehat{Err}_{train}\\quad\\quad\\quad\\quad\\quad$      ',
                           '$\\widehat{Err}_{test}\\quad\\quad\\quad\\quad\\quad$       '),
...
```{r echo=FALSE}
knitr::kable(RESULTS,
             caption = "Summary of the results of the methods used on simulated data. $\\widehat{Err}_{train}$ indicates the estimate of training error and $\\widehat{Err}_{test}$ indicates test error.",
             digits = 4,
             col.names = c('Model                            ', 
                           '$\\widehat{Err}_{train}\\quad\\quad\\quad\\quad\\quad$      ',
                           '$\\widehat{Err}_{test}\\quad\\quad\\quad\\quad\\quad$       '),
...
# Vector of standard deviations defining the shifts of the generated curves
...
```{r, echo=FALSE}
load("RData/simulace_parametry_shift_03_rf_cv.RData", verbose = F)
```
...
```{r fig.cap='Graph of the dependence of training error for individual classification methods on the value of the parameter $\\sigma_{shift}$, which defines the standard deviation for generating random vertical shifts.', warning=FALSE, eval=T}
# For training data
SIMUL_params[, 1, ] |> 
  as.data.frame() |>   
...
```{r fig.cap='Graph of the dependence of test error for individual classification methods on the value of the parameter $\\sigma_{shift}$, which defines the standard deviation for generating random vertical shifts.', warning=FALSE, eval=T}
# For test data
SIMUL_params[, 2, ] |> 
  as.data.frame() |>   
  mutate(method = methods) |>
...
```{r fig.cap='Graph of the relationship between test error rates for individual classification methods and the parameter value $\\sigma_{x}$.', warning=FALSE, eval=T}
# For test data
SIMUL_params[, 1, ] |> 
  as.data.frame() |> 
  mutate(method = methods) |>
...
```{r echo=FALSE, eval=T}
ggsave('figures/sim_03a_dependence_on_shift.pdf', width = 10, height = 6)
```
