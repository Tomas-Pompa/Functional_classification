# Závislost na parametru $\sigma_{shift}$ {#simulace3shift}
...
```{r fig.cap="Body, kterými definujeme interpolační polynomy."}
# graf bodu
...
Vidíme, že nejlépe vychází hodnota parametru $d$ jako `r d.opt[1]` pro lineární jádro s hodnotou chybovosti spočtenou pomocí 10-násobné CV `r round(1-presnost.opt.cv[1], 4)`, `r d.opt[2]` pro polynomiální jádro s hodnotou spočtenou pomocí 10-násobné CV `r round(1-presnost.opt.cv[2], 4)` a `r d.opt[3]` pro radiální jádro s hodnotou `r round(1-presnost.opt.cv[3], 4)`.
...
```{r fig.cap='Graf závislosti trénovací chybovosti pro jednotlivé klasifikační metody na hodnotě parametru $\\sigma_{shift}$, který definuje směrodatnou odchylku pro generování náhodného vertikálního posunu.', warning=FALSE, eval=T}
# pro testovaci data
SIMUL_params[, 1, ] |> 
  as.data.frame() |>   
...
```{r fig.cap='Graf závislosti testovací chybovosti pro jednotlivé klasifikační metody na hodnotě parametru $\\sigma_{shift}$, který definuje směrodatnou odchylku pro generování náhodných vertikálních posunutí.', warning=FALSE, eval=T}
# pro testovaci data
SIMUL_params[, 2, ] |> 
  as.data.frame() |>   
  mutate(method = methods) |>
