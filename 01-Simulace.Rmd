...
```{r fig.cap='Závislost odhadnutých pravděpodobností na hodnotách lineárního prediktoru. Barevně jsou odlišeny body podle příslušnosti do klasifikační třídy.'}
data.frame(
...
Spočítali jsme tedy chybovost klasifikátoru na trénovacích (`r 100 * round(1-presnost.train, 4)` %) i na testovacích datech (`r 100 * round(1-presnost.test, 4)` %).
...
Chybovost klasifikátoru na testovacích datech je tedy `r 100 * round(1-presnost.test, 4)` % a na trénovacích datech `r 100 * round(1-presnost.train, 4)` %.
...
Chybovost rozhodovacího stromu na testovacích datech je tedy `r 100 * round(1-presnost.test, 4)` % a na trénovacích datech `r 100 * round(1-presnost.train, 4)` %.
...
Chybovost rozhodovacího stromu na trénovacích datech je tedy `r 100 * round(1-presnost.train, 4)` % a na testovacích datech `r 100 * round(1-presnost.test, 4)` %.
...
Chybovost náhodného lesu na trénovacích datech je tedy `r 100 * round(1-presnost.train, 4)` % a na testovacích datech `r 100 * round(1-presnost.test, 4)` %.
...
Chybovost na trénovacích datech je tedy `r 100 * round(1-presnost.train, 4)` % a na testovacích datech `r 100 * round(1-presnost.test, 4)` %.
...
Chybovost tohoto klasifikátoru na trénovacích datech je `r 100 * round(1-presnost.train, 4)` % a na testovacích datech `r 100 * round(1-presnost.test, 4)` %.
...
V knihovně `fda.usc` využijeme funkci `classif.svm()` k aplikování metody SVM přímo pro funkcionální data.
Nejprve si vytvoříme vhodné objekty pro konstrukci klasifikátoru.
...
U všech třech jader projdeme hodnoty hyperparametru $C$ v intervalu $[10^{3}, 10^{7}]$, přičemž u jádra polynomiálního zafixujeme hyperparametr $p$ na hodnotě 3, neboť pro jiné celočíselné hodnoty metoda nedává zdaleka tak dobré výsledky. Naopak pro radiální jádro využijeme k volbě optimální hodnoty hyperparametru $\gamma$ opět `r k_cv`-násobnou CV, přičemž uvažujeme hodnoty v intervalu $[10^{-4}, 10^{1}]$. Zvolíme `coef0` $= 1$. 
...
Podívejme se, jak dopadly optimální hodnoty. Pro *lineární jádro* máme optimální hodnotu $C$ rovnu `r round(C.opt[1], 4)`, pro *polynomiální jádro* je $C$ rovno `r round(C.opt[2], 4)` a pro *radiální jádro* máme dvě optimální hodnoty, pro $C$ je optimální hodnota `r round(C.opt[3], 4)` a pro $\gamma$ je to `r round(gamma.opt, 4)`. Validační chybovosti jsou postupně `r 1 - presnost.opt.cv[1]` pro lineární, `r 1 - presnost.opt.cv[2]` pro polynomiální a `r 1 - presnost.opt.cv[3]` pro radiální jádro.
...
Chybovost metody SVM na trénovacích datech je tedy `r 100 * round(1-presnost.train.l, 6)` % pro lineární jádro, `r 100 * round(1-presnost.train.p, 6)` % pro polynomiální jádro a `r 100 * round(1-presnost.train.r, 6)` % pro gaussovské jádro.
Na testovacích datech je potom chybovost metody `r 100 * round(1-presnost.test.l, 6)` % pro lineární jádro, `r 100 * round(1-presnost.test.p, 6)` % pro polynomiální jádro a `r 100 * round(1-presnost.test.r, 6)` % pro radiální jádro.
...
U všech třech jader projdeme hodnoty hyperparametru $C$ v intervalu $[10^{0}, 10^{6}]$, přičemž u jádra polynomiálního zafixujeme hyperparametr $p$ na hodnotě 3, neboť pro jiné celočíselné hodnoty metoda nedává zdaleka tak dobré výsledky. Naopak pro radiální jádro využijeme k volbě optimální hodnoty hyperparametru $\gamma$ opět `r k_cv`-násobnou CV, přičemž uvažujeme hodnoty v intervalu $[10^{-5}, 10^{-2}]$. Zvolíme `coef0` $= 1$. 
...
U všech třech jader projdeme hodnoty hyperparametru $C$ v intervalu $[10^{-3}, 10^{4}]$, přičemž u jádra polynomiálního zafixujeme hyperparametr $p$ na hodnotě 3, neboť pro jiné celočíselné hodnoty metoda nedává zdaleka tak dobré výsledky. Naopak pro radiální jádro využijeme k volbě optimální hodnoty hyperparametru $\gamma$ opět `r k_cv`-násobnou CV, přičemž uvažujeme hodnoty v intervalu $[10^{-3}, 10^{2}]$. Zvolíme `coef0` $= 1$. 
...
Nakonec použijeme vyjádření funkcí pomocí B-splinové báze.
U všech třech jader projdeme hodnoty hyperparametru $C$ v intervalu $[10^{1}, 10^{4}]$, přičemž u jádra polynomiálního zafixujeme hyperparametr $p$ na hodnotě 3, neboť pro jiné celočíselné hodnoty metoda nedává zdaleka tak dobré výsledky. Naopak pro radiální jádro využijeme k volbě optimální hodnoty hyperparametru $\gamma$ opět `r k_cv`-násobnou CV, přičemž uvažujeme hodnoty v intervalu $[10^{-3}, 10^{2}]$. Zvolíme `coef0` $= 1$.
...
Vidíme, že nejlépe vychází hodnota parametru $d={}$ `r d.opt[1]` a $\gamma={}$ `r round(gamma.opt[1], 4)` pro lineární jádro s hodnotou chybovosti spočtenou pomocí 10-násobné CV `r round(err.opt.cv[1], 4)`, $d={}$ `r d.opt[2]` a $\gamma={}$ `r round(gamma.opt[2], 4)` pro polynomiální jádro s hodnotou chybovosti spočtenou pomocí 10-násobné CV `r round(err.opt.cv[2], 4)` a $d={}$ `r d.opt[3]` a $\gamma={}$ `r round(gamma.opt[3], 4)` pro radiální jádro s hodnotou chybovosti `r round(err.opt.cv[3], 4)`.
...
Chybovost metody SVM v kombinaci s projekcí na Reproducing Kernel Hilbert Space je tedy na trénovacích datech rovna `r 100 * round(Res[1, 2], 4)` % pro lineární jádro, `r 100 * round(Res[2, 2], 4)` % pro polynomiální jádro a `r 100 * round(Res[3, 2], 4)` % pro gaussovské jádro.
Na testovacích datech je potom chybovost metody `r 100 * round(Res[1, 3], 4)` % pro lineární jádro, `r 100 * round(Res[2, 3], 4)` % pro polynomiální jádro a `r 100 * round(Res[3, 3], 4)` % pro radiální jádro.
...
Vidíme, že nejlépe vychází hodnota parametru $d={}$ `r d.opt[1]` a $p={}$ `r poly.opt[1]` pro lineární jádro s hodnotou chybovosti spočtenou pomocí 10-násobné CV `r round(err.opt.cv[1], 4)`, $d={}$ `r d.opt[2]` a $p={}$ `r poly.opt[2]` pro polynomiální jádro s hodnotou chybovosti spočtenou pomocí 10-násobné CV `r round(err.opt.cv[2], 4)` a $d={}$ `r d.opt[3]` a $p={}$ `r poly.opt[3]` pro radiální jádro s hodnotou chybovosti `r round(err.opt.cv[3], 4)`.
...
Vidíme, že nejlépe vychází hodnota parametru $d={}$ `r d.opt[1]` pro lineární jádro s hodnotou chybovosti spočtenou pomocí 10-násobné CV `r round(err.opt.cv[1], 4)`, $d={}$ `r d.opt[2]` pro polynomiální jádro s hodnotou chybovosti spočtenou pomocí 10-násobné CV `r round(err.opt.cv[2], 4)` a $d={}$ `r d.opt[3]` pro radiální jádro s hodnotou chybovosti `r round(err.opt.cv[3], 4)`.
...
Nyní zopakujeme celou předchozí část `r n.sim`-krát a hodnoty chybovostí si budeme ukládat to listu `SIMULACE`.
...
# pro testovaci data
# sub_methods <- c('KNN', 'QDA', 'LR_functional',
#                        'RF_discr', 'RF_score', 'RF_Bbasis',
#                        'SVM linear - diskr', 'SVM linear - PCA',
#                        'SVM linear - Bbasis',
...
# ggsave("figures/kap6_sim_03_boxplot_test_new.tex", device = tikz, width = 7, height = 5)
```
...
```{r}
wilcox.test(SIMULACE$test[, 'SVM linear - Bbasis'], SIMULACE$test[, 'SVM linear - projection'], alternative = 't', paired = T)$p.value

wilcox.test(SIMULACE$test[, 'LR_functional'], SIMULACE$test[, 'SVM linear - diskr'], alternative = 't', paired = T)$p.value
```


Podívejme se na srovnání funkcionální logistické regrese a projekce na B-splinovou bázi.

```{r}
wilcox.test(SIMULACE$test[, 'LR_functional'], SIMULACE$test[, 'SVM linear - projection'], alternative = 'greater', paired = T)$p.value

wilcox.test(SIMULACE$test[, 'LR_functional'], SIMULACE$test[, 'SVM poly - projection'], alternative = 'greater', paired = T)$p.value

wilcox.test(SIMULACE$test[, 'LR_functional'], SIMULACE$test[, 'SVM rbf - projection'], alternative = 'greater', paired = T)$p.value
```

Vidíme, že lineární jádro má statisticky významně menší chybovost v porovnání s funkcionální logistickou regresí ($p$-hodnota `r wilcox.test(SIMULACE$test[, 'LR_functional'], SIMULACE$test[, 'SVM linear - projection'], alternative = 'greater', paired = T)$p.value`). Naopak u zbylých jader metody SVM se nepodařilo ukázat, že medián jejich testovacích chybovostí je  menší než medián fLR. 

Ještě by nás mohly zajímat další porovnání.

```{r}
# wilcox.test(SIMULACE$test[, 'SVM rbf - RKHS - radial'], SIMULACE$test[, 'SVM linear - diskr'], alternative = 'less', paired = T)$p.value

# wilcox.test(SIMULACE$test[, 'SVM linear - RKHS - radial'], SIMULACE$test[, 'SVM linear - diskr'], alternative = 'less', paired = T)$p.value

wilcox.test(SIMULACE$test[, 'LR_functional'], SIMULACE$test[, 'SVM linear - RKHS - radial'], alternative = 'less', paired = T)$p.value

# wilcox.test(SIMULACE$test[, 'LR_functional'], SIMULACE$test[, 'SVM rbf - RKHS - poly'], alternative = 'less', paired = T)$p.value

wilcox.test(SIMULACE$test[, 'LR_functional'], SIMULACE$test[, 'SVM rbf - RKHS - radial'], alternative = 'less', paired = T)$p.value

# wilcox.test(SIMULACE$test[, 'SVM poly - diskr'], SIMULACE$test[, 'SVM poly - Bbasis'], alternative = 'greater', paired = T)$p.value
```

Ještě se podívejme na srovnání jader u projekce.

```{r}
wilcox.test(SIMULACE$test[, 'SVM linear - projection'], SIMULACE$test[, 'SVM poly - projection'], alternative = 'less', paired = T)$p.value

wilcox.test(SIMULACE$test[, 'SVM rbf - projection'], SIMULACE$test[, 'SVM poly - projection'], alternative = 'greater', paired = T)$p.value

wilcox.test(SIMULACE$test[, 'SVM linear - projection'], SIMULACE$test[, 'SVM rbf - projection'], alternative = 'less', paired = T)$p.value
```
