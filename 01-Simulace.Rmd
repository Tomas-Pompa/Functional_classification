...
```{r fig.cap="Body definující interpolační polynomy pro obě klasifikační třídy."}
# graf bodu
...
```{r fig.cap='Znázornění dvou funkcí na intervalu $[0, 6]$, ze kterých generujeme pozorování ze tříd 0 a 1.', include=F}
...
yTrue <- t(generate_values(t, funkce_0, N, 0.2, 2))
yTrue <- rbind(yTrue, t(generate_values(t, funkce_1, N, 0.2, 2)))

# FPCA on dense set
N <- 2*N
L3 <- MakeFPCAInputs(IDs = rep(1:N, each = M), 
                     tVec = rep(t, N),
                     t(yTrue))
...
```{r include=FALSE, eval=FALSE, echo=FALSE}
set.seed(42)
ySparse <- Sparsify(yTrue, t, sparsity = c(5:8))
ySparse$yNoisy <- lapply(ySparse$Ly, function(x) x + 0.1*rnorm(length(x)))

# pdf(file = "C:/Users/Tom/OneDrive - MUNI/Diplomka/grafy_tikz/pointsplot.pdf",
...
cols <- c(rep(c('tomato', 'deepskyblue'), each = 3))
par(mfrow = c(2, 3), pty = 's', mar = c(2.5, 2.5, 0.5, 0))
i <- 1
for(ind in c(10:12+3, 101:103 + 5)) {
  CreatePathPlot(FPCAsparse, subset = ind, main = "", pch = 16, col = cols[i], cex = 0, las = 1, ylim = c(-3, 4.3), lwd = 2)
  points(FPCAsparse$inputData$Lt[[ind]], FPCAsparse$inputData$Ly[[ind]],
         col = cols[i], pch = 19, las = 1, ylim = c(-3, 4.3), xlim = c(0, 6),
       xlab = '', ylab = '')
  # lines(FPCAsparse$inputData$Lt[[ind]], FPCAsparse$inputData$Ly[[ind]],
        # col = cols[i], lwd = 1.2)
  i <- i + 1
}
...
```{r include=FALSE, eval=FALSE, echo=FALSE}
# pdf(file = "C:/Users/Tom/OneDrive - MUNI/Diplomka/grafy_tikz/designplot.pdf",
#     width = 10, # The width of the plot in inches
#     height = 5) # The height of the plot in inches

par(mfrow = c(1, 2), pty = 's', mar = c(5, 3, 3, 2))
CreateDesignPlot(L3$Lt, sort(unique(unlist(L3$Lt))), main = 'Original data')
CreateDesignPlot(ySparse$Lt, sort(unique(unlist(ySparse$Lt))), main = 'Sparse data')

# dev.off()
```

```{r include=FALSE, eval=FALSE, echo=FALSE}
# pdf(file = "C:/Users/Tom/OneDrive - MUNI/Diplomka/grafy_tikz/covplot.pdf",
#     width = 12, # The width of the plot in inches
#     height = 6) # The height of the plot in inches
par(xpd = TRUE, mfrow = c(1, 2))

CreateCovPlot(FPCAsparse, colSpectrum = c('deepskyblue2', 'tomato'), corr = T, phi = 20, theta = -20, main = '', cex = 0.7, zlab = '', legend = F, contour = T)
text3D(-1.5, 5, 0.9, expression(rho(s, t)), add = TRUE)

CreateCovPlot(FPCAsparse, colSpectrum = c('deepskyblue2', 'tomato'), corr = T, phi = 90, theta = 0, cex = 0.7, main = expression(rho(s, t)), contour = F, zlab = '')
# text3D(-2, 0.5, 0.9, expression(gamma(s, t)), add = TRUE)
...
lambda.vect <- 10^seq(from = -4, to = 1, length.out = 50) # vektor lambd
...
```{r fig.cap='Vykreslení všech vyhlazených pozorovaných křivek, barevně jsou odlišeny křivky podle příslušnosti do klasifikační třídy. Černou čarou je zakreslen průměr pro každou třídu, přerušovaně pak pro druhou třídu.', warning=FALSE}
...
```{r fig.cap='Vykreslení všech vyhlazených pozorovaných křivek, barevně jsou odlišeny křivky podle příslušnosti do klasifikační třídy. Černou čarou je zakreslen průměr pro každou třídu. Přiblížený pohled.', warning=FALSE}
DFsmooth <- data.frame(
  t = rep(t, 2 * n),
  time = rep(rep(1:n, each = length(t)), 2),
  Smooth = c(fdobjSmootheval),
...
library(fda.usc) # pro KNN, fLR, fsvm
...
Vidíme, že nejlépe vychází hodnota parametru $K$ jako `r K.opt` s hodnotou chybovosti spočtené pomocí 10-násobné CV `r round(1-presnost.opt.cv, 4)`.
